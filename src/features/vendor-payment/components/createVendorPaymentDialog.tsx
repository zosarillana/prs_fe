...
export function CreateVendorPaymentDialog({
  open,
  onOpenChange,
  onSuccess,
  payments,
  ...
  const existingPoNumbers = useMemo(
    () => new Set(payments.map((p) => p.po_no)),
    [payments],
  );

  const availablePoOptions = useMemo(() => {
    const filtered = allPoOptions.filter(
      (item) => !existingPoNumbers.has(item.po),
    );

    const seen = new Set<string>();

    return filtered.filter((item) => {
      if (seen.has(item.po)) return false;
      seen.add(item.po);
      return true;
    });
  ...
  const isAvailable = cheque_status === "available";
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[420px]">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          {/* PO Select — only shows POs not yet in vendor payments */}
          ...
          <div className="space-y-2">
            ...
            <Popover>
              <PopoverTrigger asChild>
                ...
                >
                  {vendor_id
                    ? vendors.find((vendor: any) => vendor.id === vendor_id)
                        ?.vendor_name
                    : "Select Vendor"}
                  <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                </Button>
